// /ui/i18n.js (ES Module) — Lumina Language Pack Engine v1
// ✅ 使用 /lang/{kr,cn,en,jp}.json 语言包
// ✅ 支持 t("lesson.words") 与 t("lesson_words") 兼容
// ✅ 不破坏现有 i18n.t() 调用方式
// ✅ 新增语言只需添加 /lang/es.json 等

import * as LangPack from "./core/i18n.js";

let DICT = {
  // ===== Korean =====
  kr: {
    brand: "Lumina Chinese Learning Center",
    subtitle: "루미나 글로벌 중국어 교육 플랫폼",

    nav_home: "홈",
    nav_hsk: "HSK 학습",
    nav_stroke: "한자 필순",
    nav_hanjagongfu: "한자공부",
    nav_speaking: "회화",
    nav_travel: "여행중국어",
    nav_culture: "문화",
    nav_review: "복습",
    nav_resources: "자료",
    nav_teacher: "교사 전용",
    nav_my: "내 학습",
    nav_login: "로그인",
    nav_hanja: "한자",
    login: "로그인",
    hsk_version: "버전",
    back_to_list: "목록으로",
    tab_words: "단어",
    tab_dialogue: "회화",
    tab_grammar: "문법",
    tab_ai: "AI 학습",
    tap_to_learn: "탭해서 학습",

    word_study_title: "단어 학습",
    word_study_back: "← 뒤로",
    word_study_label_pinyin: "병음",
    word_study_label_pos: "품사",
    word_study_label_meaning: "뜻",
    word_study_label_note: "의미 설명",
    word_study_label_examples: "예문",
    word_study_example_no: "예문 {n}",
    word_study_row_word: "단어(中文)",
    word_study_row_pinyin: "병음",
    word_study_row_pos: "품사",
    word_study_row_meaning: "뜻",
    word_study_section_examples: "예문",
    word_study_ex_line_zh: "중국어",
    word_study_ex_line_py: "병음",
    word_study_ex_line_tr: "번역",

    // ===== Home (page.home.js) =====
home_kicker: "✨ 오늘도 한 걸음",
home_h1: "오늘은 3분만, 중국어 해볼까요?",
home_p1: "아이는 즐겁게 배우고, 학부모/선생님은 커리큘럼을 쉽게 찾을 수 있는",
home_p1b: "중국어 학습 플랫폼",

home_cta1: "시작 학습",
home_cta2: "커리큘럼 보기",

home_tag1: "HSK 2.0/3.0",
home_tag2: "중·한·병음",
home_tag3: "따라쓰기",
home_tag4: "AI 말하기",

home_visual_title: "(여기에 메인 일러스트/캐릭터)",
home_visual_desc1: '예: "판다 + 책" / “오늘의 미션” 이미지',
home_visual_desc2: "※ 나중에 이미지 넣어도 레이아웃이 무너지지 않게 설계됨",

home_quickTitle: "빠른 시작",
home_quickSub: "아이/학부모/선생님 모두 편하게",

home_card1_badge: "아이용",
home_card1_title: "오늘의 학습",
home_card1_desc: "마지막으로 하던 수업을 이어서, 짧게라도 꾸준히!",
home_card1_btn: "바로가기",
home_card1_meta: "이번 주 3/7 완료",

home_card2_badge: "학부모/선생님",
home_card2_title: "커리큘럼 목차(검색/필터)",
home_card2_desc: "레벨/주제/기능별로 검색·필터해서 필요한 학습을 빠르게 찾을 수 있어요.",
home_card2_btn: "보기",
home_card2_chip1: "검색",
home_card2_chip2: "필터",
home_card2_chip3: "정렬",

home_card3_badge: "관리",
home_card3_title: "학부모/선생님",
home_card3_desc: "학습 진도, 숙제 제안, 예약/공지 등을 한 곳에서 확인합니다.",
home_card3_btn: "들어가기",
home_card3_chip1: "진도",
home_card3_chip2: "숙제",
home_card3_chip3: "예약",

home_twoLeftBadge: "공지/업데이트",
home_twoLeftTitle: "최근 업데이트",
home_more: "더보기",
home_upd1: "신규: HSK1 1과 「인사하기」",
home_upd2: "개선: 목차 언어 동기화",
home_upd3: "예고: 따라쓰기 ‘정답 발광/너무 잘했어요’",

home_twoRightBadge: "왜 아이에게 좋아요",
home_twoRightTitle: "학습 경험",
home_twoRightDesc: "‘단어 → 회화 → 따라쓰기 → 퀴즈’ 순서로 완주하면 성취감이 쌓여요.",
home_feat1: "AI 말하기",
home_feat1s: "자연스럽게 말하기",
home_feat2: "중·한 + 병음",
home_feat2s: "부모님도 쉽게 도와요",
home_feat3: "학습 리포트",
home_feat3s: "선생님이 추적 가능",

home_footerLine: "아이도 즐겁고, 학부모/선생님도 찾기 쉬운 중국어 학습 플랫폼",
home_contact: "문의",
home_email: "이메일",
home_privacy: "개인정보처리방침",
home_terms: "이용약관",
home_copy: "© 2026 Lumina Chinese Learning Center",

    // ===== Stroke =====
    stroke_title: "한자 필순 연습",
    stroke_desc: "한 글자를 입력하면 필순 애니메이션과 따라쓰기 연습을 할 수 있어요.",
    stroke_input_label: "한자 입력",
    stroke_input_ph: "예: 你 / 学 / 人",
    stroke_load_btn: "불러오기",

    stroke_player_title: "필순 보기",
    stroke_meaning_title: "단어 뜻 (HSK)",
    stroke_meaning_hint: "HSK 단어장에서 뜻, 병음, 예문을 보여줘요.",
    stroke_not_found: "정보를 찾지 못했어요.",
    stroke_loading: "불러오는 중…",

    stroke_modal_title: "한자 필순",
    stroke_back_to_learn: "학습으로 돌아가기",
    stroke_btn_speak: "발음",
    stroke_btn_replay: "다시",
    stroke_btn_reset: "초기화",
    stroke_btn_trace: "따라쓰기",
    stroke_demo: "데모",
    stroke_practice: "연습",
    stroke_write_here: "여기에 따라 써 보세요 ✍",
    stroke_missing_data: "이 글자의 필순 데이터가 아직 없어요.",
    stroke_continue_practice: "연습 계속 (필순 없이)",
    stroke_feedback_char: "글자 추가 요청",
    stroke_wrong_hint: "이 획부터 써 보세요",
    stroke_complete_toast: "잘했어요!",

    heroTitle: "아이부터 성인까지 사용하는 종합 중국어 학습 사이트입니다.",
    heroDesc: "HSK 학습 · 한자 필순 · 한자공부 · 회화 · 여행중국어 · 문화",

    b1: "HSK 학습",
    b2: "한자 필순",
    b3: "회화",
    b4: "여행중국어",
    b5: "문화",

    footerNote: "차근차근 완성 중: 먼저 구조를 만들고, 콘텐츠를 하나씩 채워갑니다.",

    common_loading: "불러오는 중...",
    common_retry: "다시 시도",
    common_close: "닫기",
    common_back: "목록으로",

    hsk_page_title: "HSK 학습",
    hsk_page_subtitle: "레벨을 선택하고 학습을 시작하세요",
    hsk_select_level: "레벨",
    hsk_select_version: "HSK 버전",
    hsk_search_placeholder: "단어 / 병음 / 뜻 검색",
    hsk_meta_completed: "완료",
    hsk_meta_current: "현재 학습",
    hsk_meta_review: "복습 대기",
    hsk_meta_last_study: "최근 학습",
    hsk_meta_lesson_unit: "과",
    hsk_meta_word_unit: "단어",
    hsk_directory_title: "학습 목록",
    hsk_directory_lesson: "제",
    hsk_directory_enter: "수업 시작",

    lesson_tab_vocab: "단어",
    lesson_tab_dialogue: "회화",
    lesson_tab_grammar: "문법",
    lesson_tab_extension: "확장",
    lesson_tab_practice: "연습",
    lesson_tab_ai: "AI 학습",

    practice_total: "총 문제",
    practice_question: "제",

    audio_listen: "듣기",
    audio_repeat: "따라 읽기",
    audio_speak: "발음",

hsk_tab_words: "단어",
vocab_subtitle: "본문 단어입니다. 클릭하면 발음을 들을 수 있어요.",
vocab_count: "{n}단어",
hsk_tab_dialogue: "회화",
    dialogue_subtitle: "본문 대화입니다. 중국어를 클릭하면 읽어줍니다.",
hsk_tab_grammar: "문법",
hsk_tab_extension: "확장",
hsk_tab_practice: "연습",
hsk_tab_ai: "AI 학습",
hsk_ai_tip: "오늘 배운 단어/회화를 가지고 AI에게 질문해 보세요.",
hsk_ai_placeholder: "예: ‘你好’랑 ‘您好’ 차이가 뭐예요?",
hsk_ai_send: "보내기",
hsk_ai_copy: "수업내용 복사",
hsk_ai_empty: "질문을 입력해 주세요.",
hsk_ai_prompt_preview: "AI에게 보낼 프롬프트 미리보기",
hsk_ai_next_tip: "다음 단계: AI API 연결(또는 StepRunner 연동)하면 실제 답변이 나오게 됩니다.",
    hsk_empty_lessons: "수업이 없습니다.",
    hsk_empty_dialogue: "회화 콘텐츠가 아직 없습니다.",
    hsk_empty_grammar: "문법 콘텐츠가 아직 없습니다.",
    grammar_title: "문법 포인트",
    grammar_subtitle: "이번 과에서 배우는 문법입니다.",
    hsk_extension_empty: "확장 표현이 아직 없습니다.",
    extension_title: "확장 표현",
    extension_subtitle: "본문보다 조금 더 넓게 말해 보는 표현입니다.",
    extension_badge: "확장",
    extension_tip: "듣기 → 따라 읽기 → 직접 말하기 순서로 연습하세요.",
    extension_speak: "듣기",
    hsk_review_range: "복습 범위",
    hsk_review_desc: "앞서 배운 단어와 대화를 복습해 주세요.",

    hanja_title: "한자공부",
    hanja_section_vocab: "📖 자주 쓰는 한자",
    coming_soon_detail: "한자 어휘 학습 콘텐츠가 곧 추가될 예정입니다.",
    hanja_section_compare: "🔄 중한 한자 비교",
    hanja_compare_placeholder: "간체자·번체자·한국 한자 비교 기능이 추가될 예정입니다.",

    // ===== HSK common (avoid raw keys on HSK page) =====
    hsk_title: "HSK 학습",
    hsk_header: "HSK {lv} · {version}",
    hsk_level: "레벨",
    hsk_tip: "레벨을 선택하고 수업을 시작해요.",
    hsk_search_placeholder: "단어 / 병음 / 뜻 검색",
    hsk_loading_lessons: "수업 목록 불러오는 중...",
    hsk_lesson_status: "Lesson {label} ({got}/{total})",
    hsk_back_to_list: "← 목록으로",
    hsk_words_tab: "단어",
    hsk_dialogue_tab: "회화",
    hsk_grammar_tab: "문법",
    hsk_quiz_tab: "퀴즈",
    hsk_ai_tab: "AI 학습",
    hsk_lesson_unit: "{n}과",

    practice_title: "연습",
    practice_subtitle: "문제를 풀고 제출하면 정답과 해설을 볼 수 있습니다.",
    practice_total_count: "{n}문제",
    practice_answer: "정답",
    practice_listen: "듣기",
    practice_type_choice: "선택",
    practice_empty: "연습 문제가 없습니다.",
    practice_submit: "제출하기",
    practice_total_questions: "총 {n}문제",
    practice_total_score: "총점 {score} / {total}",
    practice_question_no: "제 {n}문제",
    practice_correct: "정답",
    practice_wrong: "오답",
    practice_correct_answer: "정답은 {answer}입니다",
    practice_explanation: "해설",
    practice_unanswered: "미선택",

    review_mode: "오답 복습",
    review_current_lesson: "본과",
    review_current_level: "본급",
    review_all_wrong: "전체 오답",
    review_start: "복습",
    review_submit: "제출",
    review_completed: "복습 완료",
    review_correct: "정답",
    review_wrong: "오답",
    review_explanation: "해설",
    review_continue: "계속 복습",
    review_back_to_lesson: "목록으로",
    review_no_wrong_questions: "복습할 오답이 없습니다.",
    review_cleared: "제거된 오답",
    review_still_need_review: "남은 복습",
    review_total: "총 {n}문제",
    review_question_no: "복습 {n} / {total}",
  },

  // ===== Chinese =====
  cn: {
    brand: "Joy Chinese",
    subtitle: "AI 汉字・中文学习平台",

    nav_home: "首页",
    nav_hsk: "HSK学习",
    nav_stroke: "汉字笔顺",
    nav_hanjagongfu: "汉字功夫",
    nav_speaking: "会话",
    nav_travel: "旅游中文",
    nav_culture: "文化",
    nav_review: "复习区",
    nav_resources: "资源",
    nav_teacher: "教师专区",
    nav_my: "我的学习",
    nav_login: "登录",
    nav_hanja: "汉字",
    login: "登录",
    hsk_version: "版本",
    back_to_list: "返回目录",
    tab_words: "单词",
    tab_dialogue: "会话",
    tab_grammar: "语法",
    tab_ai: "AI学习",
    tap_to_learn: "点击学习",

    word_study_title: "单词学习",
    word_study_back: "← 返回",
    word_study_label_pinyin: "拼音",
    word_study_label_pos: "词性",
    word_study_label_meaning: "释义",
    word_study_label_note: "词义说明",
    word_study_label_examples: "例句",
    word_study_example_no: "例句 {n}",
    word_study_row_word: "词语",
    word_study_row_pinyin: "拼音",
    word_study_row_pos: "词性",
    word_study_row_meaning: "释义",
    word_study_section_examples: "例句",
    word_study_ex_line_zh: "中文",
    word_study_ex_line_py: "拼音",
    word_study_ex_line_tr: "译文",

// ===== Home (page.home.js) =====
home_kicker: "✨ 今天也前进一步",
home_h1: "今天只要3分钟，要不要学点中文？",
home_p1: "孩子学得开心，家长/老师也能轻松找到课程内容的",
home_p1b: "中文学习平台",

home_cta1: "开始学习",
home_cta2: "查看课程目录",

home_tag1: "HSK 2.0/3.0",
home_tag2: "中·韩·拼音",
home_tag3: "描红",
home_tag4: "AI口语",

home_visual_title: "(这里放主视觉/角色插画)",
home_visual_desc1: "例如：熊猫+书 / 今日任务图片",
home_visual_desc2: "※ 以后加图也不会破坏版式",

home_quickTitle: "快速开始",
home_quickSub: "孩子 / 家长 / 老师都好用",

home_card1_badge: "给孩子",
home_card1_title: "今天的学习",
home_card1_desc: "继续上次的课程，短短几分钟也能坚持！",
home_card1_btn: "立即进入",
home_card1_meta: "本周完成 3/7",

home_card2_badge: "家长/老师",
home_card2_title: "课程目录（可查找）",
home_card2_desc: "按级别/主题/技能筛选与搜索，快速找到需要的学习内容。",
home_card2_btn: "查看",
home_card2_chip1: "搜索",
home_card2_chip2: "筛选",
home_card2_chip3: "排序",

home_card3_badge: "管理",
home_card3_title: "家长/老师专区",
home_card3_desc: "学习进度、作业建议、预约与通知，在这里集中查看。",
home_card3_btn: "进入",
home_card3_chip1: "进度报告",
home_card3_chip2: "作业",
home_card3_chip3: "预约",

home_twoLeftBadge: "公告/更新",
home_twoLeftTitle: "最近更新",
home_more: "更多",
home_upd1: "新增：HSK1 第1课「打招呼」",
home_upd2: "优化：课程目录支持语言同步",
home_upd3: "预告：描红“写对发光/太棒了”",

home_twoRightBadge: "为什么适合孩子",
home_twoRightTitle: "学习体验",
home_twoRightDesc: "每节课按“词汇 → 会话 → 描红 → 测验”闯关式完成，孩子更有成就感。",
home_feat1: "AI 口语陪练",
home_feat1s: "开口更自然",
home_feat2: "中韩对照 + 拼音",
home_feat2s: "家长也能陪学",
home_feat3: "学习报告",
home_feat3s: "老师可追踪",

home_footerLine: "孩子学得开心，家长/老师也更容易找到课程内容的中文学习平台",
home_contact: "联系方式",
home_email: "邮箱",
home_privacy: "隐私政策",
home_terms: "使用条款",
home_copy: "© 2026 中文学习中心",
    // ===== Stroke =====
    stroke_title: "汉字笔顺练习",
    stroke_desc: "输入一个汉字，即可观看笔顺动画并进行描红练习。",
    stroke_input_label: "输入汉字",
    stroke_input_ph: "例如：你 / 学 / 人",
    stroke_load_btn: "加载",

    stroke_player_title: "笔顺演示",
    stroke_meaning_title: "词汇释义（HSK）",
    stroke_meaning_hint: "从HSK词库中显示拼音、韩语、例句等信息。",
    stroke_not_found: "未找到相关信息。",
    stroke_loading: "加载中…",

    stroke_modal_title: "笔画",
    stroke_back_to_learn: "返回学习",
    stroke_btn_speak: "读音",
    stroke_btn_replay: "重播",
    stroke_btn_reset: "复位",
    stroke_btn_trace: "跟写",
    stroke_demo: "演示",
    stroke_practice: "练习",
    stroke_write_here: "在这里跟着写 ✍",
    stroke_missing_data: "该字暂未收录笔画数据",
    stroke_continue_practice: "继续练习(无笔顺)",
    stroke_feedback_char: "反馈缺字",
    stroke_wrong_hint: "请从这一笔开始",
    stroke_complete_toast: "太棒了！",

    heroTitle: "适合儿童到成人使用的综合中文学习网站。",
    heroDesc: "HSK学习・汉字笔顺・汉字功夫・会话・旅游中文・文化",

    b1: "HSK 学习",
    b2: "汉字笔顺",
    b3: "会话",
    b4: "旅游中文",
    b5: "文化",

    footerNote: "逐步完善中：先把结构搭好，再把内容一块块补齐。",

    common_loading: "加载中...",
    common_retry: "重试",
    common_close: "关闭",
    common_back: "返回目录",

    hsk_page_title: "HSK学习",
    hsk_page_subtitle: "选择等级并开始课程",
    hsk_select_level: "等级",
    hsk_select_version: "HSK版本",
    hsk_search_placeholder: "搜索词语 / 拼音 / 翻译",
    hsk_meta_completed: "已完成",
    hsk_meta_current: "当前第",
    hsk_meta_review: "待复习",
    hsk_meta_last_study: "最近学习",
    hsk_directory_title: "课程目录",
    hsk_directory_lesson: "第",
    hsk_directory_enter: "进入课程",

    lesson_tab_vocab: "单词",
    lesson_tab_dialogue: "会话",
    lesson_tab_grammar: "语法",
    lesson_tab_extension: "扩展",
    lesson_tab_practice: "练习",
    lesson_tab_ai: "AI学习",

    practice_total: "题目数量",
    practice_question: "第",

    audio_listen: "听",
    audio_repeat: "跟读",
    audio_speak: "发音",

hsk_tab_words: "单词",
vocab_subtitle: "本课词汇，点击可听发音。",
vocab_count: "{n}词",
hsk_tab_dialogue: "会话",
    dialogue_subtitle: "本课会话，可点击中文朗读。",
hsk_tab_grammar: "语法",
hsk_tab_extension: "扩展",
hsk_tab_practice: "练习",
hsk_tab_ai: "AI学习",
hsk_ai_tip: "用本课单词/会话向AI提问吧。",
hsk_ai_placeholder: "例如：‘你好’和‘您好’有什么区别？",
hsk_ai_send: "发送",
hsk_ai_copy: "复制本课内容",
hsk_ai_empty: "请先输入你的问题。",
hsk_ai_prompt_preview: "发送给AI的提示词预览",
hsk_ai_next_tip: "下一步：接入AI接口（或对接 StepRunner）即可输出真实回答。",
hsk_empty_lessons: "暂无课程。",
hsk_empty_dialogue: "暂无会话内容。",
hsk_empty_grammar: "暂无语法内容。",
grammar_title: "语法要点",
grammar_subtitle: "本课学习的语法。",
hsk_extension_empty: "暂无扩展表达。",
extension_title: "扩展表达",
extension_subtitle: "这些是与本课主题相关的提高表达，可用于拓展口语。",
extension_badge: "提高",
extension_tip: "建议按「听一遍 → 跟读 → 自己说」顺序练习。",
extension_speak: "发音",
hsk_review_range: "复习范围",
hsk_review_desc: "请回顾前面学过的词汇和对话。",

    hanja_title: "韩语汉字学习",
    hanja_section_vocab: "📖 常用韩语汉字",
    coming_soon_detail: "汉字词汇学习内容即将上线。",
    hanja_section_compare: "🔄 中韩汉字对比",
    hanja_compare_placeholder: "未来将加入简体、繁体与韩字对照功能。",

    // ===== HSK common =====
    hsk_title: "HSK 学习",
    hsk_header: "HSK {lv} · {version}",
    hsk_level: "级别",
    hsk_tip: "请选择级别开始学习。",
    hsk_search_placeholder: "搜索词语 / 拼音 / 翻译",
    hsk_loading_lessons: "正在加载课程…",
    hsk_lesson_status: "第{label}课（{got}/{total}）",
    hsk_back_to_list: "← 返回目录",
    hsk_words_tab: "单词",
    hsk_dialogue_tab: "会话",
    hsk_grammar_tab: "语法",
    hsk_quiz_tab: "练习",
    hsk_ai_tab: "AI 学习",
    hsk_lesson_unit: "第{n}课",

    practice_title: "练习",
    practice_subtitle: "完成后点击提交，可查看正误与解析。",
    practice_total_count: "共 {n} 题",
    practice_answer: "正确答案",
    practice_listen: "发音",
    practice_type_choice: "选择题",
    practice_empty: "暂无练习题。",
    practice_submit: "提交答案",
    practice_total_questions: "共 {n} 题",
    practice_total_score: "总分 {score} / {total}",
    practice_question_no: "第 {n} 题",
    practice_correct: "正确",
    practice_wrong: "错误",
    practice_correct_answer: "正确答案是 {answer}",
    practice_explanation: "解析",
    practice_unanswered: "未选择",

    review_mode: "错题复习",
    review_current_lesson: "本课",
    review_current_level: "本级",
    review_all_wrong: "全部错题",
    review_start: "复习",
    review_submit: "提交",
    review_completed: "复习完成",
    review_correct: "正确",
    review_wrong: "错误",
    review_explanation: "解析",
    review_continue: "继续复习",
    review_back_to_lesson: "返回目录",
    review_no_wrong_questions: "暂无错题需要复习。",
    review_cleared: "已移除错题",
    review_still_need_review: "仍需复习",
    review_total: "共 {n} 题",
    review_question_no: "复习题 {n} / {total}",
  },

  // ===== English =====
  en: {
    brand: "Lumina Chinese Learning Center",
    subtitle: "AI Hanzi · Chinese Learning Platform",

    nav_home: "Home",
    nav_hsk: "HSK",
    nav_stroke: "Stroke Order",
    nav_hanjagongfu: "Hanzi Study",
    nav_hanja: "Hanzi",
    nav_speaking: "Speaking",
    nav_travel: "Travel Chinese",
    nav_culture: "Culture",
    nav_review: "Review",
    nav_resources: "Resources",
    nav_teacher: "Teacher",
    nav_my: "My Learning",
    nav_login: "Login",
    login: "Login",

    common_loading: "Loading...",
    common_retry: "Retry",
    common_close: "Close",
    common_back: "Back",

    hsk_page_title: "HSK Learning",
    hsk_page_subtitle: "Select a level to start learning",
    hsk_select_level: "Level",
    hsk_select_version: "HSK Version",
    hsk_search_placeholder: "Search word / pinyin / meaning",
    hsk_meta_completed: "Completed",
    hsk_meta_current: "Current Lesson",
    hsk_meta_review: "Review Words",
    hsk_meta_last_study: "Last Study",
    hsk_meta_lesson_unit: "lessons",
    hsk_meta_word_unit: "words",
    hsk_directory_title: "Course List",
    hsk_directory_lesson: "Lesson",
    hsk_directory_enter: "Enter Lesson",

    lesson_tab_vocab: "Vocabulary",
    lesson_tab_dialogue: "Dialogue",
    lesson_tab_grammar: "Grammar",
    lesson_tab_extension: "Extensions",
    lesson_tab_practice: "Practice",
    lesson_tab_ai: "AI Learning",

    practice_total: "Total Questions",
    practice_question: "Question",

    audio_listen: "Listen",
    audio_repeat: "Repeat",
    audio_speak: "Speak",

    stroke_modal_title: "Stroke",
    stroke_back_to_learn: "Back to lesson",
    stroke_title: "Stroke Order",
    stroke_btn_speak: "Speak",
    stroke_btn_replay: "Replay",
    stroke_btn_trace: "Trace",
    stroke_demo: "Demo",
    stroke_practice: "Practice",
    stroke_write_here: "Trace here ✍",
    stroke_missing_data: "Stroke data not yet available for this character.",
    stroke_continue_practice: "Continue practice (no strokes)",
    stroke_feedback_char: "Request character",
    stroke_wrong_hint: "Start from this stroke",
    stroke_complete_toast: "Great!",

    hsk_title: "HSK Learning",
    hsk_header: "HSK {lv} · {version}",
    hsk_level: "Level",
    hsk_version: "Version",
    hsk_tip: "Select a level to start.",
    hsk_search_placeholder: "Search word / pinyin / meaning",
    hsk_loading_lessons: "Loading lessons...",
    hsk_lesson_status: "Lesson {label} ({got}/{total})",
    hsk_back_to_list: "← Back to list",
    back_to_list: "Back to list",
    hsk_words_tab: "Words",
    hsk_dialogue_tab: "Dialogue",
    dialogue_subtitle: "Dialogue for this lesson. Click Chinese to hear it read aloud.",
    hsk_grammar_tab: "Grammar",
    hsk_quiz_tab: "Quiz",
    hsk_ai_tab: "AI Learn",
    hsk_lesson_unit: "Lesson {n}",

    hsk_tab_extension: "Extension",
    hsk_empty_lessons: "No lessons.",
    hsk_extension_empty: "No extension items yet.",
    extension_title: "Extension Expressions",
    extension_subtitle: "These are useful extension expressions related to this lesson.",
    extension_badge: "Plus",
    extension_tip: "Practice in this order: listen → repeat → speak by yourself.",
    extension_speak: "Speak",
    vocab_subtitle: "Vocabulary for this lesson. Click to hear pronunciation.",
vocab_count: "{n} words",
    tab_words: "Words",
    tab_dialogue: "Dialogue",
    tab_grammar: "Grammar",
    tab_ai: "AI Learn",
    tap_to_learn: "Tap to Learn",

    word_study_title: "Word Study",
    word_study_back: "← Back",
    word_study_label_pinyin: "Pinyin",
    word_study_label_pos: "Part of speech",
    word_study_label_meaning: "Meaning",
    word_study_label_note: "Sense & usage",
    word_study_label_examples: "Examples",
    word_study_example_no: "Example {n}",
    word_study_row_word: "Word (Chinese)",
    word_study_row_pinyin: "Pinyin",
    word_study_row_pos: "Part of speech",
    word_study_row_meaning: "Meaning",
    word_study_section_examples: "Examples",
    word_study_ex_line_zh: "Chinese",
    word_study_ex_line_py: "Pinyin",
    word_study_ex_line_tr: "Translation",

    grammar_title: "Grammar Points",
    grammar_subtitle: "Grammar for this lesson.",

    hsk_review_range: "Review Range",
    hsk_review_desc: "Please review the vocabulary and dialogue from previous lessons.",

    practice_title: "Practice",
    practice_subtitle: "Answer the questions and submit to see results and explanations.",
    practice_total_count: "{n} questions",
    practice_answer: "Answer",
    practice_listen: "Speak",
    practice_type_choice: "Choice",
    practice_empty: "No practice questions.",
    practice_submit: "Submit",
    practice_total_questions: "{n} questions",
    practice_total_score: "Score {score} / {total}",
    practice_question_no: "Question {n}",
    practice_correct: "Correct",
    practice_wrong: "Incorrect",
    practice_correct_answer: "Correct answer is {answer}",
    practice_explanation: "Explanation",
    practice_unanswered: "Unanswered",

    review_mode: "Wrong Answer Review",
    review_current_lesson: "This Lesson",
    review_current_level: "This Level",
    review_all_wrong: "All Wrong",
    review_start: "Review",
    review_submit: "Submit",
    review_completed: "Review Completed",
    review_correct: "Correct",
    review_wrong: "Incorrect",
    review_explanation: "Explanation",
    review_continue: "Continue Review",
    review_back_to_lesson: "Back to List",
    review_no_wrong_questions: "No wrong questions to review.",
    review_cleared: "Cleared",
    review_still_need_review: "Still need review",
    review_total: "{n} / {total}",
    review_question_no: "Review {n} / {total}",
  },

  jp: {
    common_close: "閉じる",
    word_study_title: "単語学習",
    word_study_back: "← 戻る",
    word_study_label_pinyin: "ピンイン",
    word_study_label_pos: "品詞",
    word_study_label_meaning: "意味",
    word_study_label_note: "語義の説明",
    word_study_label_examples: "例文",
    word_study_example_no: "例文 {n}",
    word_study_row_word: "語（中国語）",
    word_study_row_pinyin: "ピンイン",
    word_study_row_pos: "品詞",
    word_study_row_meaning: "意味",
    word_study_section_examples: "例文",
    word_study_ex_line_zh: "中国語",
    word_study_ex_line_py: "ピンイン",
    word_study_ex_line_tr: "訳",
  },
};

// ---------- safe storage ----------
function safeGetLS(key) {
  try { return localStorage.getItem(key); } catch { return null; }
}
function safeSetLS(key, val) {
  try { localStorage.setItem(key, val); } catch {}
}

// ---------- normalize lang codes (unified) ----------
// kr/ko → ko; cn/zh → zh; en; jp/ja → jp
function normalizeLang(input, fallback = "ko") {
  const raw = String(input || "").trim().toLowerCase();
  if (!raw) return fallback;

  if (raw === "kr" || raw === "ko" || raw === "korean" || raw.startsWith("ko")) return "ko";
  if (raw === "cn" || raw === "zh" || raw === "chinese" || raw.startsWith("zh")) return "zh";
  if (raw === "en" || raw.startsWith("en")) return "en";
  if (raw === "jp" || raw === "ja" || raw === "japanese" || raw.startsWith("ja")) return "jp";

  return fallback;
}

function canonToDictKey(canon) {
  if (canon === "ko") return "kr";
  if (canon === "zh") return "cn";
  if (canon === "jp") return "jp";
  return canon === "en" ? "en" : "kr";
}

// {name} interpolation
function interpolate(str, params) {
  if (!params) return str;
  return String(str).replace(/\{(\w+)\}/g, (_, k) => {
    const v = params[k];
    return (v === 0 || v) ? String(v) : "";
  });
}

class I18N {
  constructor() {
    // canonical: ko|zh|en
    this._lang = "ko";

    // storage
    this._storageKey = "joy_lang";

    // subscribers (legacy)
    this._handlers = new Set();

    // internal event bus
    this._bus = new Map();
  }

  init({ defaultLang = "ko", storageKey = "joy_lang" } = {}) {
    this._storageKey = storageKey || "joy_lang";

    const saved =
      safeGetLS(this._storageKey) ||
      safeGetLS("site_lang") ||
      safeGetLS("lang");

    this._lang = normalizeLang(saved || defaultLang, normalizeLang(defaultLang, "ko"));

    // keep storage consistent
    safeSetLS(this._storageKey, this._lang);
    LangPack.setLegacyDict?.(DICT);
  }

  /**
   * 加载 /lang 语言包（Language Pack Engine v1）
   * @returns {Promise<void>}
   */
  async load() {
    try {
      await LangPack.initLang?.(this._lang);
    } catch (e) {
      console.warn("[i18n] load failed:", e?.message);
    }
  }

  /** @deprecated 使用 load() */
  async loadFromJson() {
    return this.load();
  }

  // canonical getter — returns ko|zh|en|jp (kr→ko, cn→zh for compat)
  getLang() {
    const pack = LangPack.getLang?.() ?? this._lang;
    return pack === "kr" ? "ko" : pack === "cn" ? "zh" : pack;
  }

  // legacy getter (some old code expects kr/cn)
  getLangLegacy() {
    return canonToDictKey(this._lang); // kr|cn|en
  }

  // Accepts kr/cn/ko/zh/en/jp; emits change events
  async setLang(lang) {
    const next = normalizeLang(lang, "ko");
    if (next === this._lang) return this._lang;

    this._lang = next;
    safeSetLS(this._storageKey, next);

    try {
      await LangPack.setLang?.(next);
    } catch (e) {
      console.warn("[i18n] setLang load failed:", e?.message);
    }

    // legacy handlers
    for (const fn of this._handlers) {
      try { fn(this._lang); } catch {}
    }

    // internal bus
    this.emit("change", this._lang);

    // global events (pages can listen)
    this._emitWindowEvents(this._lang);

    return this._lang;
  }

  // Force without equality check (rare use)
  forceLang(lang) {
    const next = normalizeLang(lang, "ko");
    this._lang = next;
    safeSetLS(this._storageKey, next);

    for (const fn of this._handlers) {
      try { fn(this._lang); } catch {}
    }
    this.emit("change", this._lang);
    this._emitWindowEvents(this._lang);

    return this._lang;
  }

  t(key, params) {
    const fromPack = LangPack.t?.(key, params);
    if (fromPack !== key) return fromPack;

    const dictKey = canonToDictKey(this._lang); // kr/cn/en
    const pack = DICT[dictKey] || DICT.kr;
    const raw =
      (pack && key in pack) ? pack[key]
      : (DICT.kr && key in DICT.kr) ? DICT.kr[key]
      : (DICT.cn && key in DICT.cn) ? DICT.cn[key]
      : (DICT.en && key in DICT.en) ? DICT.en[key]
      : key;

    return interpolate(raw, params);
  }

  onChange(fn) {
    if (typeof fn !== "function") return () => {};
    this._handlers.add(fn);
    return () => this._handlers.delete(fn);
  }

  // internal event bus
  on(event, fn) {
    if (!event || typeof fn !== "function") return () => {};
    if (!this._bus.has(event)) this._bus.set(event, new Set());
    this._bus.get(event).add(fn);
    return () => this._bus.get(event)?.delete(fn);
  }

  emit(event, payload) {
    const set = this._bus.get(event);
    if (!set) return;
    for (const fn of set) {
      try { fn(payload); } catch {}
    }
  }

  // Apply translations to specific root (or document)
  apply(root) {
    const base = root || document;

    base.querySelectorAll("[data-i18n]").forEach((el) => {
      const key = el.getAttribute("data-i18n");
      if (!key) return;
      el.textContent = this.t(key);
    });

    base.querySelectorAll("[data-i18n-html]").forEach((el) => {
      const key = el.getAttribute("data-i18n-html");
      if (!key) return;
      el.innerHTML = this.t(key);
    });

    base.querySelectorAll("[data-i18n-placeholder]").forEach((el) => {
      const key = el.getAttribute("data-i18n-placeholder");
      if (!key) return;
      el.setAttribute("placeholder", this.t(key));
    });

    base.querySelectorAll("[data-i18n-title]").forEach((el) => {
      const key = el.getAttribute("data-i18n-title");
      if (!key) return;
      el.setAttribute("title", this.t(key));
    });

    base.querySelectorAll("[data-i18n-aria-label]").forEach((el) => {
      const key = el.getAttribute("data-i18n-aria-label");
      if (!key) return;
      el.setAttribute("aria-label", this.t(key));
    });
  }

  _emitWindowEvents(langCanon) {
    // langCanon is ko|zh|en
    try {
      window.dispatchEvent(new CustomEvent("joy:lang", { detail: { lang: langCanon } }));
    } catch {}
    try {
      window.dispatchEvent(new CustomEvent("languageChanged", { detail: { lang: langCanon } }));
    } catch {}
    try {
      window.dispatchEvent(new CustomEvent("i18n:changed", { detail: { lang: langCanon } }));
    } catch {}
  }
}

export const i18n = new I18N();

// Optional auto-init if you want (safe):
// i18n.init({ defaultLang: "ko", storageKey: "joy_lang" });
