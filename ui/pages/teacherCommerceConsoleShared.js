/**
 * 教师端 Lumina 商务控制台共享逻辑：#teacher-publishing 与 #teacher-review，本地演示数据。
 * 文案与枚举显示走 commerceDisplayLabels + i18n。
 */

/** @typedef {"publishing" | "review"} LuminaCommercePageMode */
export const PAGE_MODE_PUBLISHING = "publishing";
export const PAGE_MODE_REVIEW = "review";
...
function renderLuminaCommercePage(root, ctx) {
  const snap = ctx.snap;
  const demoUserId = ctx.demoUserId;
  const pageMode = /** @type {LuminaCommercePageMode} */ (ctx.pageMode);
  ...
  /** #teacher-publishing：按当前登录老师的 profile 过滤；审核页展示全量 listing */
  ...
  const teacherSubmitReviewActorId = sessionUserId || demoUserId;
    ...
    const gateMain = `
      ...
      <section class="card teacher-review-gate-card">
        ...
        <div class="teacher-review-gate-actions teacher-review-gate-actions--dev">
          <a class="teacher-hub-cta teacher-hub-cta--ghost" href="#teacher-publishing">${escapeHtml(
  ...
  const reviewPanel = isReviewer && (pageMode === "review" || pageMode === "publishing")
  ...
  const secondaryFoldDemoIdentity =
    pageMode === "publishing"
      ? `<div class="lts0-secondary-chunk" id="lts0-sandbox-identity" data-demo-sandbox-identity="1">
        <p class="teacher-info-note-lead lts0-chunk-banner">${escapeHtml(commerceT("commerce.stage0.profile_note"))}</p>
        <h4 class="lts0-chunk-title">${escapeHtml(commerceT("commerce.stage0.identity_title"))}</h4>
        ...
        <div class="lts0-identity-row">
          ...
          <button type="button" class="lts0-btn lts0-btn--ghost" id="lts0ResetSeed">${escapeHtml(
            commerceT("commerce.stage0.reset_seed"),
          )}</button>
        </div>
  ...
  const secondaryFoldBody = `${secondaryFoldDemoIdentity}
  ...
  const subnavActive = pageMode === "review" ? "review" : "publishing";
  const pageShellClass = pageMode === "publishing" ? "teacher-publishing" : "teacher-review";
  const stageTitle =
    pageMode === "publishing"
  ...
  const showHeroBadge = false;
  const showInlineSourceGuide = pageMode === "publishing";
  const showPublishingSourceFold = pageMode === "publishing";
  const showPrimaryDraft = pageMode !== "review";
  const showGuideColumn = pageMode === "publishing";
  ...
  const listingReviewSeparator =
    pageMode === "review" && isReviewer
      ...
      : "";
  const reviewPanelBottom = pageMode === "publishing" && isReviewer ? reviewPanel : "";
  const showSecondaryFold = pageMode === "publishing";
  ...
  const fullConsoleFooterHint =
    pageMode === "publishing"
      ? `<p class="lts0-to-full-console-hint lts0-to-full-console-hint--sales"><a href="#lts0-sandbox-advanced">${escapeHtml(
          commerceT("teacher.my_sales.link_advanced"),
        )}</a></p>`
      : "";

  const lts0Main = `
      ${demoBannerHtml(pageMode === "publishing" ? "publishing" : "review")}
      ${publishingTopBlock}
      ${
        pageMode === "publishing"
          ? ""
      ...
      ${
        showSecondaryFold
          ? `<details class="card lts0-fold-secondary" id="lts0-sandbox-advanced">
  ...
  demoSel?.addEventListener("change", () => {
    ...
    renderLuminaCommercePage(root, ctx);
  });
...
let __stage0CtxRef = /** @type {{ snap: any, demoUserId: string, pageMode: LuminaCommercePageMode } | null} */ (null);

/**
 * @param {any} ctxOrRoot
 * @param {LuminaCommercePageMode} pageMode
 */
export async function mountTeacherCommercePage(ctxOrRoot, pageMode) {
  const root =
    ctxOrRoot?.root ||
    ctxOrRoot?.app ||
  ...
  const ctx = { snap, demoUserId, pageMode };
  ...
  __stage0LangHandler = () => {
    if (__stage0RootRef?.isConnected && __stage0CtxRef) {
      renderLuminaCommercePage(__stage0RootRef, __stage0CtxRef);
    }
  };
