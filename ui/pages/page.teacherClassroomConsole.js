// #teacher-console — 课堂控制台（占位；说明从工作台首页移入本页）
...
export default async function pageTeacherClassroomConsole(ctxOrRoot) {
  const root =
    ctxOrRoot?.root ||
    ctxOrRoot?.app ||
  ...
  const main = u.isGuest
    ? `<section class="card"><p class="teacher-module-placeholder-p">${esc(t("teacher.gate.guest_body"))}</p>
        <a class="teacher-hub-cta teacher-hub-cta--primary" href="#login?next=teacher-console">${esc(t("auth.nav_login"))}</a></section>`
    : `<section class="card teacher-module-placeholder" aria-labelledby="teacher-console-h1">
        <h1 id="teacher-console-h1" class="teacher-module-placeholder-h1">${esc(t("teacher.console.title"))}</h1>
        <p class="teacher-module-placeholder-lead">${esc(t("teacher.console.desc"))}</p>
        <p class="teacher-module-placeholder-note">${esc(t("teacher.console.scope_note"))}</p>
      </section>`;
  root.innerHTML = renderTeacherAdminShell({
    active: "classroom_console",
    tx: t,
...
export function mount(c) {
  return pageTeacherClassroomConsole(c);
}
